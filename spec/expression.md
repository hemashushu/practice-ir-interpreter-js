...
XiaoXuan Lang IR 是将 XiaoXuan Lang 程序编译成目标程序（本地代码或者 WASM 字节码）过程中的中间代码，它是对目标机器（包括虚拟机 VM）指令的简单抽象和封装。

## 基本数据类型

XiaoXuan IR 只支持 `i32`, `i64`, `f32` 和 `f64` 四种基本数据类型（Primitive data type）。在 XiaoXuan IR 源码里可以直接使用数字的字面量表示这 4 种数据类型：

* 格式 `[\+\-]?[0-9]+` 会被解析为整型；
* 如果数字字面量还包含有小数点或者字符 `e` 则会解析为浮点型。
...
数字字面量整型默认是 `i64`，浮点型默认是 `f64`，如果要表示 `i32` 和 `f32`，需要在字面量后面加上 `s` 字符后缀。
...
XiaoXuan IR 还支持将 `i32` 或 `i64` 写成 `int`，编译器会根据目标机器环境自动选择，`int` 类型常用于表示内存地址、函数地址、以及用户自定义数据的地址。

## 基本语法

XiaoXuan IR 源码由 [S-表达式](https://en.wikipedia.org/wiki/S-expression) 组成，`S-表达式` 由一对圆括号里面包含一个或多个由空格分开的符号（即字符串或者数字字面量）组成。
...
结构表达式用于组织源码，相当于一般高级语言里的 "关键字"。XiaoXuan IR 的结构表达式有：
...
本地函数对应着机器指令，如整数的乘除加减运算、整数的位运算等。内置函数是运行环境所必须的基础功能，类似系统调用，比如分配内存等。

函数表达式的格式是：

`(function-name arg1 arg2 ... argN)`

表达式的第一个元素是函数名称，其余的元素是函数的参数。每条函数表达式可视作一次函数调用，每个函数 "必须" 有返回值，对于确实不具备返回意义的表达值，约定返回 `int` 类型的数字 `0`。

### 用户自定义函数

跟本地/内置函数类似，不同的是这些函数是用户产生的，一般是将本地/内置函数、结构表达式以及其他用户自定义函数按照一定的逻辑和功能要求组合而成。
...
对于函数（包括本地、内置和用户自定义函数），在运行的时候会先对各个子表达式进行求值，然后作为参数值调用函数。但对于结构表达式，并不是所有子表达式都会求值，比如 `if` 条件控制表达式，除了第二个表示条件的子表达式，其余的两个子表达式只有会被执行。

一个程序本质就是一系列函数的调用。

### IR 源码的结构

一个 IR 源文件最顶层只允许一个或并列多个 `namespace` 表达式，在 `namespace` 表达式内只允许并列一个或多个 `const`，`defn` 和 `defnr` 这 3 种表达式，结构如下：
...
在 REPL 模式下，默认在名为 `user` 的命名空间的上下文（环境）中，每一条表达式（即一对括号包围起来的一行文本）会立即被执行，但要注意，`let` 和 `set` 表达式不能直接执行，它们必须写在 `do` 表达式或者函数表达式里。

在 REPL 模式下，也可以直接输入字面量或者标识符，字面量和标识符的值会被输出。

## 命名空间表达式

XiaoXuan IR 程序实质由一堆并列的用户自定义函数和常量组成，当一个程序规模较大时，所有函数堆在一起会比较难维护。所以 XiaoXuan IR 引入了一个叫做 `命名空间`（`namespace`）的概念，将函数按照相关性划分为一个一个的 "小组"，每个 "小组" 就称为一个命名空间。
...
上面代码当中的 `foo` 是命名空间的名称，里面的 3 个表达式分别表示定义了一个常量及两个用户自定义函数。
...
不过，如果模块不同，即使名称相同，命名空间也不同，后面会详细讲述。

在运行时，每个命名空间都有（且只有）一个环境对象（Environment，可以想象为一个散列 Map），用户自定义函数及常量以平铺的形式存储在里面，存储的每项以用户自定义函数的名称或者常量的名称，即`标识符` 作为 Key。上面的示例代码所构造的环境对象如果用 JavaScript 对象来表示，大致如下：
...
注意 `namespace` 表达式是用于组织代码的一个特殊表达式，其返回值由 XiaoXuan IR 的具体编译器或者解析器（interpreter）决定，比如对于解析器，可能会返回类似上面 JavaScript 对象那样的环境对象或者环境对象的指针，而对于目标为本地代码的编译器，编译之后可能根本不存在所谓的命名空间，命名空间只是一个概念，这种情况下 `namespace` 表达式是没有返回值的。
...
上面的代码构建了三个命名空间 `foo`、`foo.bar` 和 `foo.bar.one`。这种通过将多个命名空间名称连接起来的名称也叫做 `命名空间路径`。

使用多层命名空间可以将相关功能更细致地分组，但需要注意虽然命名空间在名称上存在层次关系（或者说：父子关系），但实际上每个命名空间都是同级的、并列的、相互独立的。在运行时，子命名空间的环境并不会继承父命名空间的环境的任何标识符，不同的命名空间的里面的标识符之间没有任何关联。

### 根命名空间

命名空间路径的最前面其实还隐藏着一个命名空间————一个跟当前模块的名称同名的命名空间，也称为 `根命名空间`。
...
命名空间里的标识符的完整名称（Fullname）为：标识符所在的命名空间的完整路径 + 标识符的名称。
...
在同一个模块里，如果要访问不同的命名空间里的标识符，跟访问不同模块的标识符情况一下，需要写上标识符的完整名称。这时因为，正如前面所说，尽管父子命名空间名称上有层次关系，但实际上是相互独立的。
...
跟 `use` 表达式类似，这 3 个特殊名称会在编译之前的预处理阶段被替换成完整的路径，在编译之后这些特殊名称是不存在的。机器生成的 IR 代码总是写标识符全称的。

### 标识符名称冲突

在为标识符命名时，XiaoXuan IR 禁止以下几种情况，以防止名称冲突：
...
使用块表达式（`do` 表达式）可以将多个表达式组成一个 `表达式块`，在执行时，表达式块里的表达式将依次被执行，最后一个表达式（或者标识符、或者字面值）的值将会作为表达式块的值被返回。
...
如果常量的值为 XiaoXuan Lang 的用户自定义数据类型，因为所有 XiaoXuan Lang 的用户自定义数据都会被转换为 IR 的自定义数据和函数，XiaoXuan Lang 用户自定义数据类型的实例化过程实际上是一个构造函数的调用，所以常量指向的是一个构造函数。

为了避免每次引用常量都执行一次构造函数，通常在程序被加载之后，常量（所指向的构造函数）会先被求值，求值的结果是一个数据在内存的地址，然后使用这个地址替换常量的值。也就是说，常量指向的构造函数在应用程序的生命周期里只会被执行一次。

常量的标识符会被登记在该名称空间的环境中，任何位置都可以访问到这个常量，而且数据的引用计数值不会改变，所以常量数据不会被资源回收。

## 变量表达式

变量表达式（`let` 表达式）用于在当前有效范围（Scope）之内创建一个（局部）变量并赋值，或者说，将一个值绑定到一个`标识符`（identifier）。
...
赋值表达式会返回标识符的值，上面示例当中的表达式先创建了一个名称为 `j` 的变量，其值为 `i64` 类型的 `123`，然后又创建了一个名称为 `i` 的变量，其值为里面表达式求值后的返回值————即 `i64` 类型的 `123`，最后表达式返回 `i64` 类型的 `123`。
...
条件控制表达式有 3 个参数，每个参数都可以是一个表达式、标识符或者字面量。
...
条件表达式里的 3 个子表达式（即 3 个参数）都应该有各自的作用域，3 个作用域的标识符都应该各不相关。

但解析器或者编译器可能为了执行效率而不会在条件表达式里创建作用域，所以创建作用域的责任划分在 IR 语法里————如果在条件表达式里直接写 `let` 表达式，会被拒绝解析或编译。
...
虽然 IR 主要被编译器作为中间代码使用，但为了便于教学以及手动调试之目的，所以仍然支持注释。注释的内容会被编译器忽略。注释有两种：
...
1. `if` 表达式里的三个参数不能传 `let` 表达式，需要把 `let` 表达式放置在 `do` 里。
2. `let` 表达式里的值如果是调用 `::new(...)` 以及 `::new$N(...)` 之类的对象构造函数，需要紧接着调用 `builtin.memory.inc_ref()` 函数以增加引用，但在 `::new(...)` 以及 `::new$N(...)` 的函数里没有这个限制。当 `let` 表达式创建的标识符离开作用域时，必须调用 `builtin.memory.dec_ref()` 函数以减少引用。

3. 函数的参数不能直接传入 `::new(...)` 以及 `::new$N(...)` 之类的对象构造函数，需要使用 `let` 表达式接收此类构造函数生成的对象，然后再把它作为值传递给其他函数，同时需要遵循第 2 条。对于前端语言里直接在参数里传入新构造对象的语句，可以解析为使用 `do` 把调用函数的语句包围起来。比如：
```clojure
(do
    (let a (x::new ...))
    (let b (y::new ...))
...
4. 匿名函数不能在定义的同时直接调用，需要使用 `let` 表达式接收其值，然后再调用它或者把它作为值传递给其他函数，同时需要遵循第 2 条。
