...
class Chunk {
    ...
    static createBytes(bytesLength) {
        ...
        return new Chunk(0, 2, 0, bytesLength, data, null);
    }

    static createStruct(bytesLength, mark) {
        // 每个成员占用 8 个字节

        // `new ArrayBuffer` 方法会创建一个全 0 的字节数组
        // https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer
        let data = new ArrayBuffer(bytesLength)
        return new Chunk(0, 1, mark, bytesLength, data, null);
    }

    static createStructDestructor(bytesLength, mark, destructor) {
        // 每个成员占用 8 个字节

        let data = new ArrayBuffer(bytesLength)
        // 129 = 2^7 & 1 = 128 + 1 = 129
        return new Chunk(0, 129, mark, bytesLength, data, destructor);
    }
    ...
    /*int*/ readAddress(/*int32*/ byteOffset) {
        return this.i64Read(byteOffset);
    }
}
